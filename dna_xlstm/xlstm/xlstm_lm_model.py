...
from .modeling_xlstm_rcps import RCPSEmbedding, RCPSLMHead
...
class xLSTMLMModel(WeightDecayOptimGroupMixin, nn.Module):
    ...
    def __init__(self, config: xLSTMLMModelConfig, **kwargs):
        ...
        if config.rcps:
            self.token_embedding = RCPSEmbedding(config.vocab_size, config.embedding_dim, config.complement_map)
        else:
    ...
    def headless_forward(self, idx: torch.Tensor) -> torch.Tensor:
        x = self.token_embedding(idx)
        x = self.emb_dropout(x)
        x = self.xlstm_block_stack(x)
        return x
