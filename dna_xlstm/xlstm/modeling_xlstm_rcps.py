...
from .blocks.mlstm.layer import mLSTMLayer
from .blocks.slstm.layer import sLSTMLayer
...
class RCPSWrapper(nn.Module):
    ...
    def forward(self, x, **kwargs):
        ...
        if isinstance(self.submodule, sLSTMLayer):
            kwargs["is_rc_flipped"] = False
        fwd_out = self.submodule(x[..., :n_channels // 2], **kwargs)
        # Run submodule along rc-sequence
        if isinstance(self.submodule, sLSTMLayer):
            kwargs["is_rc_flipped"] = True
